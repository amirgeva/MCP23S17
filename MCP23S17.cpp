...
MCP23S17::MCP23S17(ispi::SPI* spi, int address)
{
  _address = (address << 1);
  _error   = OK;
  _mySPI   = spi;
...
{
  _mySPI->set_settings(ispi::BitOrder::MSB, ispi::Policy::CPOL0_FALLING);
...
bool MCP23S17::pinMode1(uint8_t pin, PinMode mode)
{
  if (pin > 15)
  {
    _error = PIN_ERROR;
...
bool MCP23S17::enableInterrupt(uint8_t pin, InterruptMode mode)
{
  if (pin > 15)
  {
    _error = PIN_ERROR;
...
bool MCP23S17::enableInterrupt16(uint16_t mask, InterruptMode mode)
{
...
{
  int e = _error;
  _error = OK;  //  reset error after read.
  return e;
}
...
//  start read
  ...
  _mySPI->transfer(MCP23S17_READ_REG | _address );
  _mySPI->transfer(reg);
  rv = uint16_t(_mySPI->transfer(0xFF)) << 8;
  rv |= _mySPI->transfer(0xFF);
  _mySPI->stop();
  return rv;
}
